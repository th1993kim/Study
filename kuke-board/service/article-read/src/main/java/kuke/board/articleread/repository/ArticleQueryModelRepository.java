...
public class ArticleQueryModelRepository {
    ...
    public Optional<ArticleQueryModel> read(Long articleId) {
        return Optional.ofNullable(stringRedisTemplate.opsForValue().get(generateKey(articleId)))
                .map(result -> DataSerializer.deserialize(result, ArticleQueryModel.class));
    }
    ...
    public Map<Long, ArticleQueryModel> readAll(List<Long> articleIds) {
        List<String> keyList = articleIds.stream()
