    ...
    public CommentResponse create(CommentCreateRequestV2 request) {
        ...
        CommentV2 comment = commentRepository.save(
                CommentV2.create(
                        snowflake.nextId(),
                        request.content(),
                        parentCommentPath.createChildCommentPath(commentRepository.findDescendantsTopPath(request.articleId(), parentCommentPath.getPath())
                                .orElse(null)),
                        request.articleId(),
                        request.writerId()));
        ...
        if (result == 0) {
            articleCommentCountRepository.save(ArticleCommentCount.create(comment.getArticleId(), 1L));
        }
